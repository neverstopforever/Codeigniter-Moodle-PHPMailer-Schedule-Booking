 ...
 */
class ClientesAkaudModel extends MagaModel {
     ...
     * @var string
     */
    public $module_campus_teachers_active;
    
    /**
     * @var string
     */
    public $module_campus_teachers_max_users;
    
    /**
     * @var string
     */
    public $module_campus_teachers_expire;
    
    /**
     * @var string
     */
    public $module_campus_students_active;
    
    /**
     * @var string
     */
    public $module_campus_students_max_users;
    
    /**
     * @var string
     */
    public $module_campus_students_expire;
    
    /**
     * @var string
     */
    public $module_campus_companies_active;
    
    /**
     * @var string
     */
    public $module_campus_companies_max_users;
    
    /**
     * @var string
     */
    public $module_campus_companies_expire;
    
    /**
     * @var string
     */
    public $emails_limit_daily;
    
    /**
     * @var string
     */
    public $emails_limit_monthly;
     ...
     * @var string
     */
    public $membership_plan; //S1

    /**
     * @var string
     */
    public $membership_type; //Starter

    /**
     * @var string
     */
    public $membership_interval; //Monthly
    ...
    public function __construct() {
        parent::__construct();
        $this->table = "clientes_akaud_accounts"; //admin_akaud db
    ...
    public function getByKey($key = null) {
        ...
        $query = "
            SELECT * FROM clientes_akaud_accounts AS ca WHERE `key` = '$key';
            ";
    ...
    public function getByIdCliente($idcliente = null) {
        ...
        $query = "
            SELECT * FROM clientes_akaud_accounts AS ca WHERE `idcliente` = '$idcliente';
            ";
        $res = $this->_db->query($query);
    ...
    public function getAll($where = null) {
        $this->_db->select("$this->table.*, clientes_akaud.cnomcli as customer_name, clientes_akaud.email as customer_email, clientes_akaud.CTFO1CLI as customer_phone, clientes_akaud.CDOMICILIO as customer_address");
    ...
    public function addEditCustomer($data = null, $type = 'add') {
        ...
        if(($type == 'edit' && !isset($data['db_table_id']) && !isset($data['db_table_client_id']))){
            return false;
        }
...
//            }
            ...
            if(isset($data['db_table_id'])) {
                $this->_db->where('id', $data['db_table_id']);
            }elseif($data['db_table_client_id']){
                $this->_db->where('idcliente', $data['db_table_client_id']);
            }
...
//        $plan = $plan_options['plan'];
        ...
        if($type == 'add'){
            $result = array(
                ...
                'trial_expire' => !empty($data['trial_expire']) ? date("Y-m-d", strtotime($data['trial_expire'])) : null,
            ...
            $result['trial_expire'] = !empty($data['trial_expire']) ? date("Y-m-d", strtotime($data['trial_expire'])) : null;
    ...
    public function getclientsDataAjax($search = null, $limit = 100){
        $this->_db->select("$this->table.key AS id, clientes_akaud.cnomcli as text");
        $this->_db->from($this->table);
        $this->_db->join('clientes_akaud', 'clientes_akaud.ccodcli = '.$this->table.'.idcliente', 'left');
        if(!empty($search)){
            $this->db->like('clientes_akaud.cnomcli', $search);
        }

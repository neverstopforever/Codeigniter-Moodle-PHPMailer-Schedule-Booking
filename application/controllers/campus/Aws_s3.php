...
class Aws_s3 extends MY_Campus_Controller
{
...
//        $this->form_validation->set_rules('fecha',"Fecha","alpha|required");
    ...
    public function updateCampusProfilePhoto($model=""){
    ...
    public function deleteCampusUserPhoto($model = null){
        $this->load->model('ProfesorModel');
        $this->load->model('AlumnoModel');
    ...
    public function resource_delete(){
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $aws_bucket = $amazon['bucket'];//$this->input->post("aws_bucket", true);
                $aws_key = $this->input->post("aws_key", true);
                $document_id = $this->input->post("document_id", true);
                $model = $this->input->post("model", true);
                $id = $this->input->post("id", true);
